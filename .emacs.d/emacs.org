...
* Font
  ...
  #+BEGIN_SRC emacs-lisp :tangle yes
    ...
    (when (eq system-type 'darwin)
      (setq preferred-fonts
            (--map (cons (car it) (+ 3 (cdr it))) preferred-fonts)))

    (let* ((f (car (-intersection
                    (-map 'car preferred-fonts) (font-family-list))))
           (f (if f
                  (concat f "-" (number-to-string
                                 (cdr (assoc f preferred-fonts))))
                nil)))
      (if f
          (progn
